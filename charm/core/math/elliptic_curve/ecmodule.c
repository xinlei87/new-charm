...
{
	/* clear structure */
	if (self->point_init && self->type == G)
	{
		debug("clearing ec point.\n");
		EC_POINT_free(self->P);
	}
	if (self->point_init && self->type == ZR)
	{
		debug("clearing ec zr element.\n");
		BN_free(self->elemZ);
	}
...
static PyObject *ECE_add(PyObject *o1, PyObject *o2)
{
	...
	else
	{
		...
		{ // if(lhs->type == G && rhs->type == ZR) or vice versa operation undefined...
...
PyTypeObject ECType = {
		PyVarObject_HEAD_INIT(NULL, 0) "elliptic_curve.Element", /*tp_name*/
...
PyTypeObject ECGroupType = {
		PyVarObject_HEAD_INIT(NULL, 0) "elliptic_curve.ECGroup", /*tp_name*/
